...
def main_worker(args):
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if 1:
                #intra cam
                features_cam, per_cam_labels = extract_features_per_cam(model, cluster_loader, print_freq=50)
                pseudo_labels_cam = OrderedDict()
                cluster_cam = OrderedDict()
                dominant_node  = OrderedDict()
                cluster_features_cam = OrderedDict()
                fname_cam = {}
                memory_c = {}
                for cam in features_cam.keys():
                    fname_cam[cam] =list(features_cam[cam].keys())
                    features_cam[cam] = torch.cat([features_cam[cam][f].unsqueeze(0) for f in features_cam[cam].keys()], 0)
                    feat_dists, feat_nbrs = get_dist_nbr(features=F.normalize(features_cam[cam], dim=1).cpu().numpy(), k=args.k1, knn_method='faiss-gpu')
                    #pseudo_labels_cam[cam], dominant_node[cam] = cluster_by_infomap(feat_nbrs, feat_dists, min_sim=args.eps/2, cluster_num=args.k2, node_flag=2)
                    rerank_dist = compute_jaccard_distance(features_cam[cam], k1=args.D_k1, k2=args.D_k2)
                    Ag_cam = AgglomerativeClustering(n_clusters=len(features_cam[cam])//5,
                                    affinity="precomputed",
                                    linkage='average')
                    pseudo_labels_cam[cam] = Ag_cam.fit_predict(rerank_dist)
                    ...
                    '''cluster_features_cam = generate_cluster_features(pseudo_labels_cam[cam], features_cam[cam])
                    memory_cam = ClusterMemory(model.module.num_features, len(cluster_cam[cam]), temp=args.temp,
                               momentum=args.momentum, use_hard=args.use_hard).cuda() 
                    memory_cam.features = F.normalize(cluster_features_cam, dim=1).cuda()
                    memory_c[cam] = memory_cam
                trainer.memory_c = memory_c
                pseudo_labeled_dataset_cam = []
                for f, _, cid in sorted(dataset.train):
                    label = pseudo_labels_cam[cid][fname_cam[cid].index(f)]
                    pseudo_labeled_dataset_cam.append((f, label.item(), cid))
                train_loader_cam = get_train_loader(args, dataset, args.height, args.width,
                                        args.batch_size, args.workers, args.num_instances, iters,
                                        trainset=pseudo_labeled_dataset_cam) 
                train_loader_cam.new_epoch()'''
                count = 0
                #decay = 0.5*(1+math.cos(math.pi*epoch/(3*args.epochs)))
                for label, node in infomation_node.items():
                #for node, _ in enumerate(pseudo_labels):
                    cid = camid[node]
                    filename = fname[node]
                    ...
                    for nnode in global_cluster:
                        if camid[nnode] != cid:
                            ...
                            if nnode_index_cam in local_cluster:
                                ...
                                '''if pseudo_labels_cam[cid][nnode_index_cam] != -1:
                                    label_cam = []
                                    for node_cam in cluster_cam[cid][pseudo_labels_cam[cid][nnode_index_cam]]:
                                        label_cam.append(pseudo_labels[fname.index(fname_cam[cid][node_cam])])
                                    pseudo_labels[nnode] = Counter(label_cam).most_common(1)[0][0]'''
                                #r = random.random()
                                #if r <= decay:
                                    #dominant_index = dominant_node[cid][pseudo_labels_cam[cid][nnode_index_cam]]
                                    #pseudo_labels[nnode] = pseudo_labels[fname.index(fname_cam[cid][dominant_index])]
                                #else:
        ...
        #trainer.train(epoch, train_loader, optimizer, print_freq=args.print_freq, train_iters=len(train_loader), data_loader_cam = train_loader_cam)
