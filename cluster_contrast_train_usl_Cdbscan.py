...
import pandas as pd
from clustercontrast.utils import data
...
from clustercontrast import cons
...
from clustercontrast.cons.DBScanner import DBScanner
from clustercontrast.cons.Active_learning import Active_learning
from clustercontrast.cons.AdditionalFunction import AdditionalFunction
...
def main_worker(args):
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            features, data_label= extract_features(model, cluster_loader, print_freq=50)
            ...
            if epoch == 0:
                ...
                df = pd.DataFrame.from_dict(data_label, orient='index',columns=['label'])
                data_frame = df.reset_index().rename(columns = {'index':'fname'})
                ml=ad.get_must_link(data_frame,0)
                cl=ad.get_connot_link(data_frame,0)
                if consnum > 0:
                    ml = ad.increase_ml(ml, int(consnum / 2))
                    cl = ad.increase_cl(cl, int(consnum / 2))
            db = DBScanner(features, data_frame, eps=args.eps, min_pts=4)
            cluster_count = db.dbscan(ml, cl)
            print("Number of clusters found:", cluster_count)
            #db.getlabel()
            print("constrain number:", consnum)
            pseudo_labels = db.evaluation(data_frame)
            #num_cluster = len(set(pseudo_labels)) - (1 if -1 in pseudo_labels else 0)
            num_cluster = cluster_count
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'logs/market_cons'))
