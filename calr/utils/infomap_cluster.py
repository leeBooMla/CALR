...
from calr.utils.infomap_utils import Timer
import collections
    ...
    for i in tqdm(range(nbrs.shape[0])):
        ...
        for j in range(0, len(nbrs[i])):
            ...
            if i == nbrs[i][j]:
                ...
                score[i] += 1/(dists[i][j]+mean) #得分函数
            else:
                break
        # 统计孤立点
        if count == 0:
            single.append(i)
    return single, links, score


def cluster_by_infomap(nbrs, dists, min_sim, cluster_num=2, node_flag=0, cam_id = None):
    """
    基于infomap的聚类
    :param nbrs:
    :param dists:
    ...
    with Timer('get links', verbose=True):
        single, links, score = get_links(single=single, links=links, nbrs=nbrs, dists=dists, min_sim=min_sim)
    ...
    if node_flag == 1:
        information_node = {}
        Cam_proxy = {}
        clusters = get_cluster(pre_labels)
        for key, value in clusters.items():
            ...
            for node in value:
                if score[node]>max:
                    ...
                    max = score[node]
            if max > 72:
                information_node[key] = inode'''
            mean = np.mean(list(score.values()))
            for node in value:
                if score[node] > mean:
                        information_node[key] = node
            #median = np.median(list(score.values()))
            '''for node in value:
                if cam_id[node] not in Cam_proxy[key].keys():
                    Cam_proxy[key][cam_id[node]] = []
                Cam_proxy[key][cam_id[node]].append(node)
            for cid, proxy in Cam_proxy[key].items():
                max = 0
                if len(proxy) > 4:
                    for node in proxy:
                        if score[node]>max:
                            ...
                            max = score[node]
                    information_node[key] = inode'''
        return pre_labels, information_node
        ...
        for key, value in clusters.items():
            ...
            for node in value:
                if score[node]>max:
                    ...
                    max = score[node]
            if max > 0:
                dominant_node[key] = inode
        return pre_labels, dominant_node
